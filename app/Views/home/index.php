...
<div class="app-shell app-shell--home">
        ...
        <?php
        $omEyebrow = trim((string) ($operating['eyebrow'] ?? ''));
        $omTitle = trim((string) ($operating['title'] ?? ''));
        $omSub = trim((string) ($operating['subtitle'] ?? ''));
        $omLead = trim((string) ($operating['lead'] ?? ''));
        $eyebrowLine = $omEyebrow !== '' ? $omEyebrow : $omSub;
        ...
        $omImages = [];
        foreach (isset($operating['images']) && is_array($operating['images']) ? $operating['images'] : [] as $row) {
            if (! is_array($row)) {
                continue;
            }
        ...
        $opsHeadingId = 'home-ops-heading';
        ?>
        <section
            class="app-band section home-blocks-ops home-blocks-ops--wash<?= $omImages !== [] ? ' home-blocks-ops--has-visual' : '' ?>"
            aria-labelledby="<?= e($opsHeadingId) ?>"
            data-reveal
            data-home-ops
        >
            <div class="shell shell--fluid">
                <div class="home-blocks-ops__layout">
                    <?php if ($omImages !== []): ?>
                    <div
                        class="home-blocks-ops__mosaic<?= count($omImages) === 1 ? ' home-blocks-ops__mosaic--single' : '' ?><?= count($omImages) === 2 ? ' home-blocks-ops__mosaic--pair' : '' ?>"
                        data-ops-mosaic
                    >
                        <?php foreach ($omImages as $mi => $shot): ?>
                        <?php
                        $tier = $mi === 0 ? 'hero' : ($mi === 1 ? 'a' : 'b');
                        $alt = $shot['alt'] !== '' ? $shot['alt'] : ($omTitle !== '' ? $omTitle . ' — image ' . (string) ($mi + 1) : 'Approach — image ' . (string) ($mi + 1));
                        ?>
                        <figure class="home-blocks-ops__frame home-blocks-ops__frame--<?= e($tier) ?>">
                            <img
                                class="home-blocks-ops__img"
                                src="<?= e($shot['image']) ?>"
                                alt="<?= e($alt) ?>"
                                width="<?= $tier === 'hero' ? 900 : 640 ?>"
                                height="<?= $tier === 'hero' ? 1200 : 480 ?>"
                                loading="<?= $mi === 0 ? 'eager' : 'lazy' ?>"
                                decoding="async"
                                sizes="(min-width: 1100px) 42vw, (min-width: 640px) 46vw, 100vw"
                            >
                        </figure>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <div class="home-blocks-ops__editorial">
                        <header class="home-blocks-ops__intro">
                            <?php if ($eyebrowLine !== ''): ?>
                            <p class="eyebrow home-blocks-ops__eyebrow"><?= e($eyebrowLine) ?></p>
                            <?php endif; ?>
                            <?php if ($omTitle !== ''): ?>
                            <h2 id="<?= e($opsHeadingId) ?>" class="section__title home-blocks-ops__title"><?= e($omTitle) ?></h2>
                            <?php else: ?>
                            <h2 id="<?= e($opsHeadingId) ?>" class="visually-hidden">Our approach</h2>
                            <?php endif; ?>
                            <?php if ($omLead !== ''): ?>
                            <p class="home-blocks-ops__lead"><?= e($omLead) ?></p>
                            <?php endif; ?>
                        </header>

                        <?php if ($steps !== [] || $hlTitle !== '' || $hlBody !== ''): ?>
                        <div class="home-blocks-ops__lower">
                            <?php if ($steps !== []): ?>
                            ...
                            <aside class="home-blocks-ops__highlight">
                                <div class="home-blocks-ops__highlight-rail" aria-hidden="true">
                                    <span class="home-blocks-ops__highlight-orbit"></span>
                                    <span class="home-blocks-ops__highlight-bar"></span>
                                </div>
                                <div class="home-blocks-ops__highlight-copy">
                                    <?php if ($hlTitle !== ''): ?>
